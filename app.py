...
def get_weather_forecast(*args: tuple):
    ...
    try:
        ...
        #weather_api_key = "e139fbd22cdbe3efb6319047030141c5"
        open_weather_map = OWM('dbf1c716cecbf43263218e4e0505968c')
...
def current_time(*args: tuple):
    """
    Вывод текущего времени
    """
    current_time = datetime.now().time()
    print(colored(f"{current_time.hour} h {current_time.minute} min", "red"))
    play_voice_assistant_speech(translator.get("Now on the clock"))

    if current_time.minute < 10:
        play_voice_assistant_speech(translator.get("{0} hour 0{1} minutes").format(str(current_time.hour), str(current_time.minute)))
    else:
        play_voice_assistant_speech(translator.get("{0} hour {1} minutes").format(str(current_time.hour), str(current_time.minute)))
...
commands = {
    ...
    ("language", "язык", "russian", "английский"): change_language,
    ...
    ("time", "время", "clock", "часы"): current_time,
}
